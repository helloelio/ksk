...
let createCard = (
  ...
  document.querySelector(`.edit-btn-${id}`).addEventListener('click', () => {
    ...
    document.querySelector('.edit-card').addEventListener('click', () => {
      ...
      arr[0].selectOptionType = document.querySelector('#edit-select').value;
...
//! open modal window (create card window)
let openCardMenu = () => {
...
//! close modal window(create card window) by click on "x"
...
//! close modal window(create card window) by press "Escape"
...
//! check function. if checkbox true you can drag and drop cards
